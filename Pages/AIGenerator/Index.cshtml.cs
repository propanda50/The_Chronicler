...
namespace TheChronicler.Web.Pages.AIGenerator
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostGenerateEntityName([FromBody] GenerateEntityNameRequest request)
        {
            try
            {
                // Use AI service to generate unique name
                var name = await _aiGameMasterService.GenerateRandomNameAsync(
            ...
            {
                // Fallback to extensive list
                var fallbackName = _aiGenerationService.GetExtensiveFallbackName(
                    request.EntityType ?? "character",
                    request.Theme ?? "fantasy"
                );
                return new JsonResult(new { name = fallbackName });
            }
        }

        public async Task<IActionResult> OnPostGenerateDescription([FromBody] GenerateDescriptionRequest request)
        {
            try
            {
                var description = await _aiGameMasterService.GenerateDescriptionAsync(
                    request.EntityType ?? "character",
                    request.EntityName ?? "",
                    request.Details
                );
    ...
    public class GenerateEntityNameRequest
    {
        public string? EntityType { get; set; } // campaign, session, event, location, character, npc
        public string? Theme { get; set; } // fantasy, scifi, horror, etc.
